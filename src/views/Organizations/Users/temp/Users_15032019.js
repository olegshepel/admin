...
export default class Users extends React.PureComponent {
  ...
  updateItem(index) {this.setParams('update', `Edit User`, false, index);}
  ...
  renderModal() {
    if (this.state.modalMarker == 'create') {
      return (
        <Form
        roles={this.state.data}
        organization_id={this.state.orgID}
         />
      );
    }
    if (this.state.modalMarker == 'update') {return (<Form />);}
  ...
  render() {
    return (
      ...
      {this.renderModal()}
        ...
        <HeaderBlock createItem={this.createItem.bind(this)}/>
        </div>
